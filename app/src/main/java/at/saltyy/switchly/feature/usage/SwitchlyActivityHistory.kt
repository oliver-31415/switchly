...
object SwitchlyActivityHistory {
    enum class Source {
        SCHEDULE,
        LOCATION,
        WIFI,
    ...
    fun entriesForDay(context: Context, dayMillis: Long, limit: Int = 40): List<Entry> {
        ...
        return AppLogStore.latestLines(context, 1000)
    ...
    private fun dedupeGenericTemporaryEntries(entries: List<Entry>): List<Entry> {
        ...
        if (specificTempSeconds.isEmpty()) return entries
    ...
    private fun parseLine(context: Context, line: String): Entry? {
        ...
        if (tagStart < 0 || tagEnd <= tagStart) return null
    ...
    private fun blockingEntry(context: Context, timeMillis: Long, message: String, lowerMessage: String): Entry? {
        if ("app_block" !in lowerMessage && "website_block" !in lowerMessage && "surface_block" !in lowerMessage && "multiwindow_block" !in lowerMessage) return null
    ...
    private fun locationEntry(context: Context, timeMillis: Long, message: String, lowerMessage: String): Entry? {
        if ("geofence" !in lowerMessage && "location" !in lowerMessage) return null
        if ("registered" in lowerMessage || "failed" in lowerMessage) return null
        return simpleEntry(
            timeMillis,
    ...
    private fun scanEntry(
        ...
        if ("action applied" !in lowerMessage) return null
        val action = valueAfter(message, "action=").orEmpty()
    ...
    private fun fallbackActionLabel(context: Context, action: String): String {
        ...
        if (clean.isBlank()) return context.getString(R.string.activity_history_action_unknown)
        return clean.substring(0, 1).uppercase(Locale.getDefault()) + clean.substring(1)
    }

    private fun readableDuration(raw: String): String {
        if (raw.equals("ask", ignoreCase = true)) return raw
